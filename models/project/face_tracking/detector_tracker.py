...
def apply_nms(dets, iou_thresh=NMS_IOU_THRESHOLD):
    if len(dets) == 0:
        return dets
...
def filter_detections(dets):
    ...
    if len(filtered) > MAX_DETECTIONS_PER_FRAME:
        order = filtered[:, 4].argsort()[::-1]
        filtered = filtered[order[:MAX_DETECTIONS_PER_FRAME]]
...
def detect_faces_multiscale(frame, detector, device, frame_idx):
    ...
    if frame_idx % TILE_DETECT_INTERVAL == 0:
        tiles = [
            (0, 0, w // 2, h // 2),
            (w // 2, 0, w, h // 2),
            (0, h // 2, w // 2, h),
